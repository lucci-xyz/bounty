...
## System Overview
- **Frontend:** Next.js 15 App Router renders all user interfaces under `app/`, backed by shared context (`components/Providers`, `components/NetworkProvider`) and a minimal design system rooted in `app/globals.css`.
- **Backend:** Next.js API routes in `app/api/*` act as the HTTP edge. Each handler delegates logic to reusable modules in `server/` (auth, blockchain, GitHub, database, notifications).
- **Persistence:** Prisma (`prisma/schema.prisma`) targets Postgres and is accessed exclusively through query helpers in `server/db/prisma.js`.
...
- **Integrations:** A GitHub App (via `server/github/*`) drives installations, issue comments, and webhook automation, while Resend powers alert emails.

## Component Map

### Frontend (Next.js App Router)
- Global structure (`app/layout.jsx`) wraps every page with wallet/network providers, the shared `Navbar`, and `Footer`.
- `components/Providers.jsx` boots wagmi, RainbowKit, and TanStack Query. Chains are limited to Base Sepolia and Mezo Testnet, matching the registry defaults.
- `components/NetworkProvider.jsx` fetches `/api/registry`, persists the preferred env in the `network_env` cookie through `/api/network/env`, and exposes `useNetwork` helpers so pages can render the correct token metadata and RPC hints.
- Main routes:
...
### API Edge Layer
- Each handler in `app/api/*/route.js` is thin: validate inputs, call server helpers, respond with JSON.
- Groups:
...
- API routes execute in the Next.js runtime (edge or Node depending on the handler). Heavy work is pushed into `server/` so routes stay declarative and testable.

### Server Modules
| Path | Responsibility |
| --- | --- |
| `server/config.js` | Loads env vars, builds wallet map per network alias, exposes token metadata, and validates required configuration. |
| `server/auth/siwe.js` | Generates nonces and constructs/verifies SIWE messages against the configured domain. |
| `server/db/prisma.js` | Initializes Prisma and exports query objects for bounties, wallets, PR claims, users, stats, and allowlists. |
| `server/blockchain/contract.js` | Wraps ethers.js providers, network registry lookups, bounty ID computation, payout execution, and token helpers. |
| `server/github/client.js` | Boots the GitHub App via `octokit`, returning installation-scoped clients plus helpers for comments and labels. |
| `server/github/webhooks.js` | Central dispatcher for `issues`, `pull_request`, and `ping` events: posts CTA buttons, records PR claims, triggers `resolveBountyOnNetwork`, and escalates failures via comments + email. |
| `server/notifications/email.js` | Sends alert emails through Resend (no-op when credentials are missing). |
...
- `server/db/prisma.bountyQueries` enforces derived fields such as `environment` (from `CONFIG.envTarget`) and BigInt conversions before returning plain JS objects.
...
- `server/blockchain/contract.js` creates per-alias clients with ethers v6, computes bounty IDs, formats/parse token amounts, and handles resolver wallet selection. It also handles non-1559 networks (Mezo) by sending legacy gas parameters.
...
### GitHub Integration
- The GitHub App manifest lives in `github-app-manifest.json`. Runtime credentials are loaded in `server/config.js`.
...
- Environment variables drive everything (`env.txt` documents defaults). `server/config.validateConfig()` ensures essentials (session secret, GitHub keys, wallet keys, registry defaults) exist at boot.
- Network aliases are toggled per user via the `network_env` cookie and persisted server-side using `/api/network/env`.
- Sessions rely on `iron-session` (`lib/session.js`), scoped to the domain derived from `FRONTEND_URL`, and marked secure in production.
- `NEXT_PUBLIC_*` vars configure wallets (WalletConnect project ID), dummy data, and local/testnet toggles.

## Observability & Safety
- SIWE signatures (`server/auth/siwe.js`) ensure wallet ownership before linking.
- GitHub webhooks are HMAC-verified, and sensitive endpoints (allowlists, dashboard APIs) require an active GitHub session.
- `server/blockchain/validation.js` normalizes addresses, bytes32 IDs, and transaction hashes before any on-chain call.
...
- Security-sensitive secrets (GitHub private key, resolver private keys) are never bundled at build time; `server/config.js` lazy-loads them from file paths or env vars.

## Repository Layout & Tooling
- `app/`: Next.js routes (UI + API).
- `components/`: shared UI and context providers aligned with the accent palette.
- `server/`: backend helpers described above.
...
- `dummy-data/`: mocked responses for UI development.
- `lib/`: vendored dependencies (OpenZeppelin 5.x, forge-std) and misc helpers.
...
- `docs/reference/server.md`: deep dive into `server/` modules.
