...
const testnetNetworks = createFlagDeclaration({
  ...
  description: 'Exposes Base Sepolia network in the UI.',
  defaultValue: false, // Disabled for production; enable via FLAGS_LOCAL_OVERRIDES for staging
  options: [
...
const multiTokenFeature = createFlagDeclaration({
  ...
  defaultValue: false, // Disabled until additional tokens are allowed on mainnet
  options: [
    { label: 'Off', value: false },
    { label: 'On', value: true }
  ]
...
const betaProgramEnabled = createFlagDeclaration({
  name: 'betaProgramEnabled',
  key: 'beta-program-enabled',
  description: 'Requires beta approval before maintainers can access the GitHub app or create bounties.',
  defaultValue: false,
  options: [
