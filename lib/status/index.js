...
// Matches `uint64 public constant RESOLVE_GRACE = 1 days;` in
// contracts/current/BountyEscrow.sol. `refundExpired` reverts with
// DeadlineNotReached until block.timestamp > deadline + RESOLVE_GRACE, so
// this app-side mirror must add the same window or it tells a sponsor a
// refund is ready up to a day before the chain will actually accept it.
export const RESOLVE_GRACE_SECONDS = 24 * 60 * 60;

/**
 * Check if a bounty is eligible for refund.
 * Eligibility requires: open status + deadline past the resolver's grace
 * window (RESOLVE_GRACE). Before that window closes the resolver may still
 * pay, and `refundExpired` would revert.
 */
export function isRefundEligible(bounty, nowSeconds = Math.floor(Date.now() / 1000)) {
  ...
  // Strictly past deadline + RESOLVE_GRACE, mirroring the contract's
  // refundExpired guard (block.timestamp > deadline + RESOLVE_GRACE). At or
  // before that boundary an on-chain refund would revert with
  // DeadlineNotReached, so it is not yet eligible.
  return Number.isFinite(deadline) && nowSeconds > deadline + RESOLVE_GRACE_SECONDS;
}
